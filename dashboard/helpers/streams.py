...
def top_streamers(streams: pl.DataFrame, latest: pl.DataFrame, n: int) -> pl.DataFrame:
  ...
  online_streams = (
    streams.filter(pl.col("stream_id").is_in(online_stream_ids["stream_id"]))
    .sort("ingestion_ts")
    .with_columns(
      pl.col("viewer_count")
      ...
      .round(decimals=0)
      .cast(pl.Int32)
      .alias("avg_viewers_10")
    )
  )

  # Aggregate by user_name and sort by rolling mean
  return (
    online_streams.sort("viewer_count", descending=True)
    .group_by("user_name")
    .agg(
...
def top_tags_by_frequency(tags: pl.DataFrame, n: int) -> pl.DataFrame:
  ...
  return tags.group_by("tag_name").agg(pl.len().alias("uses")).sort("uses", descending=True).head(n)
...
def top_tags_by_viewers(tags: pl.DataFrame, streams: pl.DataFrame, n: int) -> pl.DataFrame:
  ...
  return (
    tags.join(streams_df, on=["stream_id", "ingestion_ts"])
    .group_by("tag_name")
...
def top_longest_streams(latest: pl.DataFrame, top_n: int) -> pl.DataFrame:
  ...
  return (
    latest.drop_nulls(subset=["stream_time_seconds"])
