...
def rating_popularity(quadrants: pl.DataFrame, n: int):
  with st.container(border=True):
    ...
    with c1:
      ...
      gems = quadrants.filter(pl.col("quadrant") == "Hidden gem").sort("total_rating", descending=True).head(n)
    ...
    with c2:
      ...
      overhyped = quadrants.filter(pl.col("quadrant") == "Overhyped").sort("current_viewers", descending=True).head(n)
