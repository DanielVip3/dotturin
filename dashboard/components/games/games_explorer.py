...
def games_explorer(games: pl.DataFrame):
  with st.container(border=True):
    ...
    all_themes = sorted(games.select("themes").explode("themes").drop_nulls().unique().to_series().to_list())

    all_platforms = sorted(games.select("platforms").explode("platforms").drop_nulls().unique().to_series().to_list())
