...
def write_to_clickhouse(df: DataFrame, table: str):
  df.write.format("clickhouse").option("host", CH_HOST).option("protocol", "http").option("http_port", CH_PORT).option(
    "database", CH_DB
  ).option("table", table).option("user", CH_USER).option("password", CH_PASSWORD).mode("append").save()
...
def process_gold(batch_df: DataFrame, _: int):
  ...
  clean_batch_df = batch_df.filter(col("game_name").isNotNull() & (expr("trim(game_name)") != "")).fillna(
    {"user_name": "Unknown", "language": "Unknown"}
  )

  clean_batch_df.persist()

  # Streamer dimension table
  dim_streamer_df = clean_batch_df.select("user_name").distinct().withColumn("streamer_id", expr("xxhash64(user_name)"))

  write_to_clickhouse(dim_streamer_df, "dim_streamer")

  # Language dimension table
  dim_language_df = clean_batch_df.select("language").distinct().withColumn("language_id", expr("xxhash64(language)"))
  ...
  dim_game_df = (
    clean_batch_df.select("game_name")
    .distinct()
  ...
  dim_date_df = (
    ...
    .distinct()
    .withColumn(
      "date_id",  # number like 20260707
      concat(col("date_year"), lpad(col("date_month"), 2, "0"), lpad(col("date_day"), 2, "0")).cast("long"),
    )
  )
  ...
  fact_stream_hourly_df = (
    clean_batch_df.withColumn("game_id", expr("xxhash64(game_name)"))
...
query = (
  silver_streams_df.writeStream.outputMode("update")
